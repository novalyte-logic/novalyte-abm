...
import {
  ...
  ArrowDownRight, CheckCircle2, DollarSign, BarChart3, Siren,
} from 'lucide-react';
...
type AppView = 'dashboard' | 'keywords' | 'clinics' | 'crm' | 'voice' | 'campaigns' | 'email' | 'forecast' | 'leads' | 'analytics' | 'aiengine';
...
function IntelligenceFeed({
  ...
  const typeIcon = (insight: AIInsight) => {
    if (insight.severity === 'critical') return AlertTriangle;
    if (insight.type === 'risk') return Siren;
    if (insight.type === 'opportunity') return TrendingUp;
    if (insight.type === 'action') return Zap;
    return Brain;
  };

  const typeClass = (insight: AIInsight) => {
    if (insight.severity === 'critical') return 'text-red-300 bg-red-500/10 border-red-500/30';
    if (insight.type === 'risk') return 'text-amber-300 bg-amber-500/10 border-amber-500/30';
    if (insight.type === 'opportunity') return 'text-emerald-300 bg-emerald-500/10 border-emerald-500/30';
    return 'text-cyan-300 bg-cyan-500/10 border-cyan-500/30';
  };

  if (!primaryInsight) {
    return (
      <div className="px-5 py-10 text-center rounded-xl border border-white/[0.06] bg-black/35">
        <Sparkles className="w-8 h-8 text-slate-700 mx-auto mb-2" />
        <p className="text-sm text-slate-400">AI feed will appear once pipeline signals are available.</p>
      </div>
    );
  }

  return (
    <div className="rounded-xl border border-white/[0.06] bg-black/35 overflow-hidden">
      <div className="px-6 py-5 border-b border-white/[0.05]">
        <div className="border-l-2 border-amber-400/80 pl-4 space-y-2">
          <div className="flex items-center gap-2">
            <span className={cn('inline-flex items-center rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide', typeClass(primaryInsight))}>
              Priority Insight
            </span>
            {primaryInsight.metric && (
              <span className="text-[10px] text-slate-400 rounded-md border border-white/10 px-2 py-0.5">{primaryInsight.metric}</span>
            )}
          </div>
          <h4 className="text-[28px] leading-tight font-semibold text-slate-50">{primaryInsight.title}</h4>
          <p className="text-sm leading-6 text-slate-400 max-w-4xl">{primaryInsight.description}</p>
          {primaryInsight.actionLabel && primaryInsight.actionTarget && (
              <button
              onClick={() => onNavigate(primaryInsight.actionTarget as AppView)}
              className="inline-flex items-center gap-2 mt-1 rounded-lg bg-cyan-400 text-black px-4 py-2 text-xs font-semibold transition-all hover:bg-cyan-300 shadow-[0_0_18px_rgba(34,211,238,0.28)]"
            >
      ...
      <div className="divide-y divide-white/[0.05] max-h-[420px] overflow-auto">
        {secondaryInsights.map((insight) => {
          const Icon = typeIcon(insight);
          return (
            <div key={insight.id} className="px-6 py-4 flex items-start gap-3 hover:bg-white/[0.02] transition-colors">
              <div className={cn('w-8 h-8 mt-0.5 rounded-lg border flex items-center justify-center shrink-0', typeClass(insight))}>
                <Icon className="w-4 h-4" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-2">
                  <p className="text-xl leading-tight text-slate-100 font-medium">{insight.title}</p>
                  {insight.metric && (
                    <span className="text-[10px] text-slate-400 rounded-md border border-white/10 px-2 py-0.5 shrink-0">{insight.metric}</span>
                  )}
                </div>
                <p className="text-sm text-slate-400 mt-2 leading-6">{insight.description}</p>
              </div>
              {insight.actionLabel && insight.actionTarget && (
                <button
                  onClick={() => onNavigate(insight.actionTarget as AppView)}
                  className="shrink-0 rounded-lg bg-cyan-400 text-black px-4 py-2 text-xs font-semibold transition-all hover:bg-cyan-300 shadow-[0_0_14px_rgba(34,211,238,0.25)]"
                >
                  {insight.actionLabel}
                </button>
              )}
        ...
        <div className="col-span-12 lg:col-span-7 glass-card overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-100">AI Intelligence Feed</h3>
          ...
          <div className="p-4">
            <IntelligenceFeed insights={insights} onNavigate={setCurrentView} />
          </div>
        </div>
